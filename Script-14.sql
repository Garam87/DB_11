/*
	...
	DCL (Data Control Language) : 데이터를 다루기 위한 다루는 언어
...
CREATE USER ogr_sample IDENTIFIED BY sample1234;
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO ogr_sample;
...
GRANT CREATE TABLE TO ogr_sample;

ALTER USER ogr_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM; -- 할당
...
GRANT CONNECT, RESOURCE TO ogr_sample;
...
-- kh / ogr_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh) ogr_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명
GRANT SELECT ON EMPLOYEE TO ogr_sample;

-- 3. (sample) 다시 kh.EMPLOYEE 조회
SELECT * FROM kh.EMPLOYEE;
--SELECT * FROM kh.DEPARTNENT; --> DEPARTMENT 테이블 조회 권한은 준적 없으므로 불가!
...
REVOKE SELECT ON EMPLOYEE FROM ogr_sample;
